    ...
    for (i = 0; i < m_btn_config.btn_count; i++)
    {
        if (m_btn_data[i] != m_hw_params[i].idle_level)
        {
            if (m_btn_config.get_tick_cb() - m_btn_hold_count[i] >= APP_BTN_HOLD_SO_LONG_TIME_MS)
            {
                ...
                {
                    if (m_btn_config.get_tick_cb() - m_btn_on_hold_count[i] >= APP_BTN_ON_HOLD_TIME_FIRE_EVENT_MS)
                    {
        ...
        if ((m_btn_press_evt >> i) & 0x01)
        {
            ...
            if (m_btn_config.get_tick_cb() - m_btn_last_press_time[i] >= APP_BTN_DOUBLE_CLICK_TIME_MS)
            {
                m_btn_press_cnt[i] = 0;
            }
            else
